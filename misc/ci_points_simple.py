...
# orthogonal to the x=y=z line, with the origin at (0,0,0)
...
output_dir = os.path.join('output', f'ci_points_simple_{timestamp}')
...
# Define the radius of the circle for the CI points
radius = 0.5
...
# Calculate the CI points using the basis vectors
ci_point1 = origin + radius * (np.cos(theta1) * basis2 + np.sin(theta1) * basis3)
ci_point2 = origin + radius * (np.cos(theta2) * basis2 + np.sin(theta2) * basis3)
ci_point3 = origin + radius * (np.cos(theta3) * basis2 + np.sin(theta3) * basis3)

# Store all points for easier access
points = [origin, ci_point1, ci_point2, ci_point3]
point_labels = ['Origin (0,0,0)', 'CI Point 1', 'CI Point 2', 'CI Point 3']
...
for i in range(1, 4):
    axes[0].plot([origin[0], points[i][0]], [origin[1], points[i][1]], 
                color=point_colors[i], linestyle='-', alpha=0.7)

# Plot the circle in the XY plane
theta = np.linspace(0, 2*np.pi, 100)
circle_xy = np.array([radius * np.cos(theta), radius * np.sin(theta)])
# Project the circle onto the XY plane
circle_xy_proj = np.zeros((2, len(theta)))
for i in range(len(theta)):
    point_3d = radius * (np.cos(theta[i]) * basis2 + np.sin(theta[i]) * basis3)
    circle_xy_proj[0, i] = point_3d[0]
    circle_xy_proj[1, i] = point_3d[1]

axes[0].plot(circle_xy_proj[0, :], circle_xy_proj[1, :], 'k--', alpha=0.5, label='Circle')
...
# Draw lines connecting origin to each CI point
for i in range(1, 4):
    axes[1].plot([origin[0], points[i][0]], [origin[2], points[i][2]], 
                color=point_colors[i], linestyle='-', alpha=0.7)

# Project the circle onto the XZ plane
circle_xz_proj = np.zeros((2, len(theta)))
for i in range(len(theta)):
    point_3d = radius * (np.cos(theta[i]) * basis2 + np.sin(theta[i]) * basis3)
    circle_xz_proj[0, i] = point_3d[0]
    circle_xz_proj[1, i] = point_3d[2]

axes[1].plot(circle_xz_proj[0, :], circle_xz_proj[1, :], 'k--', alpha=0.5, label='Circle')
...
# Draw lines connecting origin to each CI point
for i in range(1, 4):
    axes[2].plot([origin[1], points[i][1]], [origin[2], points[i][2]], 
                color=point_colors[i], linestyle='-', alpha=0.7)

# Project the circle onto the YZ plane
circle_yz_proj = np.zeros((2, len(theta)))
for i in range(len(theta)):
    point_3d = radius * (np.cos(theta[i]) * basis2 + np.sin(theta[i]) * basis3)
    circle_yz_proj[0, i] = point_3d[1]
    circle_yz_proj[1, i] = point_3d[2]

axes[2].plot(circle_yz_proj[0, :], circle_yz_proj[1, :], 'k--', alpha=0.5, label='Circle')
...
# Draw lines connecting origin to each CI point
for i in range(1, 4):
    ax.plot([origin[0], points[i][0]], [origin[1], points[i][1]], [origin[2], points[i][2]], 
           color=point_colors[i], linestyle='-', alpha=0.7)
