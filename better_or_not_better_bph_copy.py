...
def compute_berry_phase(eigvectors_all, theta_vals):
    ...
    if not path_is_closed:
        print("Path is not closed. Need to close it...")
    else:
        print("Path is closed.")    
    # Store original theta_vals for return
    #theta_vals_orig = theta_vals
    
    # Compute tau and gamma
    N, M, _ = eigvectors_all.shape # N is the number of points, M is the number of eigenvectors and their components
    ...
    for n in range(M):
        for m in range(M):
            for i in range(N): 
                ...
                gamma_increment = (np.imag(tau[n, m, i]) + np.imag(tau[n, m, i-1])) / 2 * delta_theta_integrate
                gamma[n, m, i] = gamma[n, m, i-1] + gamma_increment
    
    return np.real(tau), np.real(gamma)
