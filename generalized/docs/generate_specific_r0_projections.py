#!/usr/bin/env python3
"""
Script to generate R_0 plane projections specifically for the combined effect figures.
This script creates subplots showing the projections on the R_0 plane for the three
combined effect configurations.
"""
...
def generate_combined_effect_r0_projections(R_0, filename_suffix):
    """Generate R_0 plane projections for the combined effect figure"""
    # Define parameter values for combined effect
    ...
    for i, (d, theta) in enumerate(zip(combined_d_values, combined_theta_values)):
        if i < len(axes):
            plot_r0_projection_subplot(axes[i], R_0, d, theta, f'd = {d}, θ = {theta:.2f}')
    
    # Add a main title
    R_0_str = ', '.join(map(str, R_0))
    fig.suptitle(f'R_0 Plane Projections for Combined Effect with R_0=({R_0_str})', fontsize=16)
    ...
    # Save the figure
    output_filename = os.path.join(output_dir, f'r0_projections_combined_effect_{filename_suffix}.png')
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f"Saved {output_filename}")

def main():
    """Main function to generate R_0 plane projections for the combined effect figures"""
    # Generate R_0 plane projections for the three combined effect files
    generate_combined_effect_r0_projections((0, 0, 0), 'R0_0_0_0')
    generate_combined_effect_r0_projections((1, 1, 1), 'R0_1_1_1')
    generate_combined_effect_r0_projections((0, 0, 2), 'R0_0_0_2')
